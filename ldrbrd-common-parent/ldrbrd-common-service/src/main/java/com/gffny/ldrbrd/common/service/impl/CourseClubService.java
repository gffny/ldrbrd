...
public class CourseClubService implements ICourseClubService {
	...
	public Course getCourseByNameAndTeeColour(String courseName,
		...
		try {
			...
			List<Course> courseList = courseDao.findByNamedQuery(Course.FIND_BY_NAME_AND_TEE_COLOUR, params);
			if (courseList != null) {
	...
	/*
	 * (non-Javadoc)
	 * @see com.gffny.ldrbrd.common.service.ICourseClubService#getFavouriteCourseList(java.lang.String, int)
	 */
	...
	public List<Course> getFavouriteCourseList(String golferId,
		...
		if(StringUtils.isNotEmpty(golferId)) {
			...
			try {
				if(favouriteLimit > 0) {
					result = courseDao.findByNamedQuery(Course.FAVOURITE_LIST_BY_GOLFER_ID, params, favouriteLimit);
				} else {
					result = courseDao.findByNamedQuery(Course.FAVOURITE_LIST_BY_GOLFER_ID, params);
				}
			} catch (DataAccessException e) {
				LOG.error(e.getMessage());
	...
	public Course createCourse(String courseName, Club club,
		...
		try {
			...
			params.put("courseName", courseName);
			//TODO use the super.namedQueryResultOrNullMethod
			List<Course> courseList = courseDao.findByNamedQuery(Course.FIND_BY_CLUB_ID_AND_COURSE_NAME, params);
			if(courseList != null && courseList.size() > 0 && courseList.get(0) != null) {
				result = courseList.get(0);
			} else {
				result = Course.createCourse(courseName, club, teeColour, slopeIndex, par, courseImageReference);
	...
	public Club createClub(final String clubName) {
		...
		try {
			...
			//TODO use the super.namedQueryResultOrNullMethod
			List<Club> clubList = clubDao.findByNamedQuery(Club.FIND_CLUB_BY_CLUB_NAME, params);
			if(clubList != null && clubList.size() > 0 && clubList.get(0) != null) {
	...
	/*
	 * (non-Javadoc)
	 * @see com.gffny.ldrbrd.common.service.ICourseClubService#saveOrUpdateHole(com.gffny.ldrbrd.common.model.impl.CourseHole)
	 */
	public void saveOrUpdateHole(final CourseHole courseHole) {
		...
		try {
			//TODO use the super.namedQueryResultOrNullMethod
			existingCourseList = courseHoleDao.findByNamedQuery(CourseHole.FIND_BY_COURSE_ID_AND_HOLE_NUMBER, params);
			if(null != existingCourseList && 0 < existingCourseList.size() && null != existingCourseList.get(0)) {
				//update
				LOG.error("NOT IMPLEMENTED ****************");
			} else if (null == existingCourseList || 0 == existingCourseList.size() || null == existingCourseList.get(0)) {
				//persist
