...
public class GolfPlayController extends AbstractController {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/competitionscorecard")
	...
	public ResponseEntity<JsonResponse<JSONable>> startNonCompetitionScorecard(
		...
		if (null != scorecardRequest
			...
			// populate the map
			golferScorecardMap
					.put(scorecardRequest.getScoreKeeperId(),
							scorecardService.startGeneralScorecard(
									scorecardRequest.getScoreKeeperId(),
									scorecardRequest.getCourseId(), null, null)
									.getId());
			// check if there are other golfers (which there should be from a
			// business logic perspective)
			if (ArrayUtils.isNotEmpty(scorecardRequest.getGolferArray())) {
				for (String golferId : scorecardRequest.getGolferArray()) {
					golferScorecardMap.put(
							golferId,
							scorecardService.startGeneralScorecard(golferId,
									scorecardRequest.getScoreKeeperId(),
									scorecardRequest.getCourseId(), null, null)
									.getId());
				}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/holescorelist")
	// , method = RequestMethod.GET)
	 ...
	 * @return
	 */
	@RequestMapping(value = "/submitscorecard")
	// , method = RequestMethod.GET)
	public ResponseEntity<JsonResponse<JSONable>> submitScorecard(
			@RequestBody final ScorecardRequest scorecardRequest) {
